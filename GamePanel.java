...
//    X and Y coordinates of snake.
    ...
    public void startGame(){
        //When we start the game, we will call newApple method to create a new apple on
        newApple();
        running = true;//As it is running now so boolean running is set to be true.
        ...
        if (running){
            ...
            for (int i = 0; i <bodyParts ; i++) {
                if (i == 0) {
                    ...
                    g.setColor(new Color(random.nextInt(255),random.nextInt(255),random.nextInt(255)));// Colourful snake
                    g.fillRect(x[i],y[i],UNIT_SIZE,UNIT_SIZE);
                }
            }
            g.setColor(Color.red);
    ...
    public void checkCollisions(){
        // This checks if the head collide wit the body
        for (int i = bodyParts; i >0 ; i--) {
            if (x[0]==x[i]&& y[0]==y[i]){
                running =false;
            }
